        ...
        if (albumsMissingArt.moveToFirst()) {
            ...
            do {
                ...
                Cursor songs = context.getContentResolver().query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
                        ...
                        new String[] { String.valueOf(albumsMissingArt.getLong(0)) },
                        MediaStore.Audio.Media.TRACK);

                if (!songs.moveToFirst()) continue;

                Log.w("Testing", "---------------" + songs.getString(0));
                ...
                if (images != null && images.length > 0) {
                    ...
                    String[] filesToScan = new String[allFiles.length];

                    for (int i = 0; i < allFiles.length; i++)
                        filesToScan[i] = allFiles[i].getAbsolutePath();
